...
from code.stage_4_code.Setting_Mini_Batch import Setting_Mini_Batch
...
if 1:
    ...
    fs=[
        ...
        nn.Sequential(
            ...
            # we input embedding_dim, assume 1 item in batch, then we have the LSTM neuron
            # process one word (one embedding dim) at a time
            nn.LSTM(embedding_dim, hidden_dim, batch_first=True, num_layers=2),
    ...
    gs=[nn.Sequential(
            nn.Linear(int((600000/2400)*sequence_length), 11),
    ...
    # ---- running section ---------------------------------
    method_obj = Method_RNN('rnn', '', epoch, learning_rate, device,f,g).to(device)
    print('************ Start ************')
