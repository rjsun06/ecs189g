...
class Method_RNN(method, nn.Module):
    data = None
    batch_size=0
    def __init__(self, mName, mDescription, _max_epoch, _learning_rate, device, f, g):
        self.device=device
    ...
    def train(self, X, y):
        ...
        #print(x)
        y_true=torch.LongTensor(y).to(self.device)
    ...
    def test(self, X):
        ...
        y_pred = self.forward(torch.LongTensor(X).to(self.device))
    ...
    def run(self):
        self.batch_size=len(self.data['train']['y'])
